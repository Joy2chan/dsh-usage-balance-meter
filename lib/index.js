...
function resolveConfig(config) {
    ...
    const priceTable = {
        ...
        ...config.prices?.providers !== undefined
            ? {
                providers: Object.fromEntries(Object.entries(config.prices.providers).map(([provider, table]) => [
                    provider,
                    {
...
function buildLedgerSummary(ledger, resolved) {
    ...
    if (resolved.budget.enabled) {
        ...
        if (period === 'day')
            used = totals.today.cost;
        else if (period === 'month')
            used = totals.month.cost;
        else if (period === 'all')
            used = totals.all.cost;
        else {
