...
async function resolveOpenCodeGoKey(ctx, resolved) {
...
async function queryOpenCodeGoQuota(ctx, resolved, signal) {
    ...
    if (!response.ok)
        throw new Error(`OpenCode Go quota HTTP ${response.status}`);
    const data = await response.json();
    const usage = data.usage;
    if (usage === null || typeof usage !== 'object')
        throw new Error('OpenCode Go usage missing');
    return {
...
async function renderCostCommand(ctx, resolved, invocation, signal, ledger) {
    ...
    if (summary.budget !== null) {
        ...
        if (b.percent >= b.errorPercent)
            lines.push('  ⚠️ Over budget: you have exceeded the configured budget.');
        else if (b.percent >= b.warnPercent)
            lines.push('  ⚠️ Approaching budget: you are near the configured budget.');
    }
    else {
 ...
 */
export function bindUsageMeter(ctx, ledger, resolved) {
    const usageMeter = {
        typertRemote: undefined,
        async getState() {
            ;
            ctx.logger?.info?.('[usage-balance-meter] getState called');
...
export function apply(ctx, config) {
    ...
    ctx.provide?.('usageMeter', bindUsageMeter(ctx, ledger, resolved));
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(_args, exec) {
            ...
            const warnings = [];
            if (summary.budget !== null) {
                if (summary.budget.percent >= summary.budget.errorPercent)
                    warnings.push('Budget exceeded');
                else if (summary.budget.percent >= summary.budget.warnPercent)
                    warnings.push('Approaching budget');
            }
            return {
