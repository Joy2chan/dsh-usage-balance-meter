...
面向 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 的插件：注册一个模型可调用的 `deepseek_api_status` 工具，用来读取：

- 当前接入的 DeepSeek API 账户余额（`GET /user/balance`）
- 当前会话的提供方 token 用量
- 可选的账户级用量响应（可配置网关端点）
- 可选的会话费用估算（可配置每百万 token 单价）
...
- `/cost` 斜杠命令
- 按 provider 聚合用量（所有 active `ctx.llm` provider 的调用次数 + token）
